...
std::vector<double> Diffusion(std::vector<double> z, std::vector<double> f, double dt) {
  ...
  for (size_t i = 1; i < N - 1; ++i) {
    if (i == 1) {
      centralDiagonal.at(i - 1) = 1. + 0.5 * alpha;
    } else {
  ...
  for (size_t i = 1; i < N - 1; ++i) {
    ...
    rhs.at(i - 1) = dt * Q / 2.;
    if (i == 1) {
      rhs.at(i - 1) += 0.5 * alpha * f.at(i + 1) + (1. - 0.5 * alpha) * f.at(i) + alpha * (diskSize * dz / D);
    } else {
  ...
  fNew[0] = fNew[1] + diskSize * dz / D * Q_0;
  return fNew;
}
...
std::vector<double> Advection(std::vector<double> z, std::vector<double> f, double dt) {
  ...
  std::fill(lowerDiagonal.begin(), lowerDiagonal.end(), -0.5 * gamma);
  std::fill(upperDiagonal.begin(), upperDiagonal.end(), 0.);

  for (size_t i = 0; i < N - 1; ++i) {
    if (i == 0) {
      centralDiagonal.at(i) = 1.;  //  + 0.5 * gamma;
    } else {
      centralDiagonal.at(i) = 1. + 0.5 * gamma;
    }
  }
  ...
  for (size_t i = 0; i < N - 1; ++i) {
    ...
    rhs.at(i) = dt * Q / 2.;
    if (i == 0) {
      rhs.at(i) += gamma * (diskSize * dz / D) + (1.) * f.at(i);
    } else {
      rhs.at(i) += 0.5 * gamma * f.at(i - 1) + (1. - 0.5 * gamma) * f.at(i);
    }
  }
  ...
  for (size_t i = 0; i < N - 1; ++i) fNew[i] = solution[i];
  return fNew;
}
...
double runSim(int zOrder, std::string outputFilename) {
  ...
  const auto z = NM::linspace(0., NM::HaloSize, zSteps);
